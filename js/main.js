...
var myGenres = filterGenres(movies)
...
function rendarAll(){
    ...
    moviesSelectEl.addEventListener("change",(event)=>{
        ...
        let foundedGenres = findGenres(event.target.value,myGenres)
        let genreselectEl = renderSelecttt(foundedGenres)
        containerEl.appendChild(genreselectEl)
    })
...
function findGenres(id,filmArr){
    let founded = []
    filmArr.forEach(value => value.id == id ? founded.push(value): '')
    return founded
}
...
function filterFilm(moviesArr){
    ...
    function filterGenres(moviesArr){
        let tempMoviesArr = []
    
        moviesArr.forEach((value,index) => {
            tempMoviesArr.push({id:value.id,genres:value.genres})
        }) 
        return tempMoviesArr
}
...
function renderSelecttt(arr){
    ...
    arr.forEach((value,index) =>{
        let optionEl = document.createElement('option')
        optionEl.textContent = value.genres
        optionEl.value = value.id
        parentSelectEl.appendChild(optionEl)
    })
    return parentSelectEl
